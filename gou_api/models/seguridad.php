<?php
	...
	class Seguridad extends Connection {
		...
		function generaToken($dispositivo, $usuario){
			...
			$consulta = " SELECT id FROM api_token_gou WHERE token = :token;";
			$resultados = $this->queryArray($consulta, array("token" => $token));
			if($resultados["total"] > 0){
				return $this->generaToken($dispositivo, $usuario, $inicio, $fin);
			}else{
				$consulta = " UPDATE api_token_gou SET activo = 0 WHERE dispositivo = :dispositivo AND activo = 1;";
				...
				$consulta = " INSERT INTO api_token_gou (id, id_empleado, dispositivo, token, inicio, fin) VALUES ";
		...
		function validaToken($dispositivo, $token){
			$consulta = " SELECT id, id_empleado AS empleado, dispositivo, inicio, fin FROM api_token_gou WHERE token = :token AND activo = 1;";
			$resultados = $this->queryArray($consulta, array("token" => $token));
			if($resultados["total"] == 1){
				...
				if(crypt($texto, $token) == $token){
					...
					if(strtotime($registro["fin"]) < strtotime(date("Y-m-d H:i:s"))){
						$consulta = " UPDATE api_token_gou SET activo = 0 WHERE id = :id;";
		...
		function terminaUsuario($token){
			$consulta = " UPDATE api_token_gou SET activo = 0 WHERE id = ". $this->Sesion .";";
