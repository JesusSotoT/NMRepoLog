	...
	class Seguridad extends Connection {
		...
		function validaParametros($parametros, $validaciones){
			foreach ($validaciones as $index => $validacion) {
				if(($validacion["tipo"] == "imagen" || $validacion["tipo"] == "video" || $validacion["tipo"] == "zip") && !Input::tieneArchivo($index) && !$validacion["nulo"]){
					return array("status" => false, "mensaje" => "El parametro ::". $index .":: no puede ser vacio o nulo000");
				}else if(($validacion["tipo"] != "imagen" && $validacion["tipo"] != "video" && $validacion["tipo"] != "zip") && ((!array_key_exists($index, $parametros) && !$validacion["nulo"]) || (!array_key_exists($index, $parametros) && $parametros[$index] == "" && !$validacion["vacio"]))){
					return array("status" => false, "mensaje" => "El parametro ::". $index .":: no puede ser vacio o nulo333");
					...
					switch ($validacion["tipo"]) {
						...
						case 'video':
							...
							break;
						case 'zip':
							if(!Input::esZip($index)) return $error;
							break;
					}
				}
			}
		...
		function generaToken($dispositivo, $usuario){
			...
			$consulta = " SELECT id FROM api_token_checador WHERE token = :token;";
			$resultados = $this->queryArray($consulta, array("token" => $token));
			if($resultados["total"] > 0){
				return $this->generaToken($dispositivo, $usuario, $inicio, $fin);
			}else{
				$consulta = " UPDATE api_token_checador SET activo = 0 WHERE dispositivo = :dispositivo AND activo = 1;";
				...
				$consulta = " INSERT INTO api_token_checador (id, id_empleado, dispositivo, token, inicio, fin) VALUES ";
		...
		function validaToken($dispositivo, $token){
			$consulta = " SELECT id, id_empleado AS empleado, dispositivo, inicio, fin FROM api_token_checador WHERE token = :token AND activo = 1;";
			$resultados = $this->queryArray($consulta, array("token" => $token));
			if($resultados["total"] == 1){
				...
				if(crypt($texto, $token) == $token){
					...
					if(strtotime($registro["fin"]) < strtotime(date("Y-m-d H:i:s"))){
						$consulta = " UPDATE api_token_checador SET activo = 0 WHERE id = :id;";
		...
		function terminaUsuario($token){
			$consulta = " UPDATE api_token_checador SET activo = 0 WHERE id = ". $this->Sesion .";";
