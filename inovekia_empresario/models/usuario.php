    ...
    class UsuarioModel extends Connection
    {
        ...
        public function iniciarSesionUsuario($dispositivo, $usuario, $contrasena){
            ...
            if($login["status"]){
                ...
                $login["rows"][0]["perfil"] = 2;
                $login["rows"][0]["razon"] = $usuario;

                $this->connect(true);
                $fecha_ultimo_acceso = date('Y-m-d H:i:00', time());
                $consulta = "UPDATE customer SET fechaultimoacceso = :fecha WHERE instancia = :cliente;";
                $consulta = $this->queryArray($consulta, array("fecha" => $fecha_ultimo_acceso, "cliente" => $cliente));
            }
            return $login;
        }
