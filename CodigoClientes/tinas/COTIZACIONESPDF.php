	...
	class CFDIPDF{
		...
		function generar($imagen, $plantilla=0, $positionPath="",$facturacion=true){
			...
			if(isset($_SESSION['caja']['descGeneral'])) {
				...
				$content.='<div style="margin-left:540px;margin-top:'.$px.'px"><table><tr><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';border-right:1px dashed '.$this->color.';text-align:left;width:100px">Subtotal</td><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';text-align:right;width:100px">'.$this->total[0].' '.$this->datosCFD[3].'</td></tr>';  // .$this->datosCFD[69].'</td></tr>';
			}
			//print_r($this->carray);
			...
			foreach($this->carray as $key=>$indiceKey){
				if($key=='IVAR' || $key=='ISR' || $key=='RTP'){
					...
					foreach($indiceKey as $index => $value){
						if(isset($value['Valor'])) {
							$down.='<tr><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';border-right:1px dashed '.$this->color.';text-align:left">'.$key.' ('.number_format($index,2).'%)</td><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';text-align:right">'.number_format((float)$value['Valor'],2,'.',',').' '.$this->datosCFD[3].'</td></tr>'; //.$this->datosCFD[69].'</td></tr>';
						} else {
				...
				if($key=='IVA' || $key == 'IEPS'){
					...
					foreach($indiceKey as $index => $value){
						if (isset($value['Valor'])) {
							$up.='<tr><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';border-right:1px dashed '.$this->color.';text-align:left">'.$key.' ('.number_format($index,2).'%)</td><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';text-align:right">'.number_format((float)$value['Valor'],2,'.',',').' '.$this->datosCFD[3].'</td></tr>';  // .$this->datosCFD[69].'</td></tr>';
						} else {
			...
			switch($plantilla){
				case 0:
					...
					$content.='<tr><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';border-right:1px dashed '.$this->color.';text-align:left">Total</td><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';text-align:right">'.$this->total[1].' '.$this->datosCFD[3].'</td></tr></table></div><div style="margin-left:10px"><table><tr><td style="font-size:11px;border-bottom:1px dashed '.$this->color.';border-right:1px dashed '.$this->color.';padding-left:30px;padding-right:10px">TOTAL EN LETRA:</td><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';padding-left:5px">'.$this->metodoPago[2].$this->datosCFD[3].'</td></tr></table></div><div style="font-weight:bold;font-size:11px;background:'.$this->color.';margin-top:10px;color:#000000;">Observaciones</div><div style="font-size:10px;border:1px solid '.$this->color.';padding:5px"><div style="width:381px;padding-top:-2px">'.wordwrap($this->observaciones,112,"</div><div style=\"width:380px;margin-top:3px\">",true).'</div></div><div>'.'<div></div><div></div><div style="font-weight:bold;font-size:13px;background:'.$this->color.';padding-right:10px">T&eacute;rminos y condiciones</div><div style="font-size:11px;background:'.$this->color.';padding-right:10px; text-align:right;"></div><div><table style="border:1px '.$this->color.';"><tr><td style="font-size:10px; text-align: right; width:750px"></td></tr><tr></tr><tr></tr><tr><td style="font-size:10px; text-align: justify; width:750px">'.wordwrap('Pagar el 50% al hacer el pedido, y liquidar antes de enviar su mercanc&iacute;a o al recibirla por parte de la empresa. <br />Si requiere factura debe solicitarla al hacer el pedido con su vendedor. <br />Cambios o cancelaciones: Despu&eacute;s de haber realizado el pago se le har&aacute; un cargo de penalizaci&oacute;n seg&uacute;n sea el caso por gastos ocasionados por el mismo. <br />La entrega de la tina ser&aacute; de 2 a 8 d&iacute;as h&aacute;biles m&aacute;s lo que tarde el env&iacute;o a pie de casa. <br />Toda la mercanc&iacute;a viaja por cuenta y riesgo del comprador. <br />Garant&iacute;a de 3 a&ntilde;os en f&aacute;brica. <br /> <br />').'</td></tr></table></div><div></div><div></div><div></div>'.'</div></body></html>';
					break;

				case 1:
					// RCA - Se agrego la opcion de Terminos y condiciones al final del documento						
					$content.='<tr><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';border-right:1px dashed '.$this->color.';text-align:left">Total</td><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';text-align:right">'.$this->total[1].' '.$this->datosCFD[3].'</td></tr></table></div><div style="margin-left:10px"><table><tr><td style="font-size:11px;border-bottom:1px dashed '.$this->color.';border-right:1px dashed '.$this->color.';padding-left:30px;padding-right:10px">TOTAL EN LETRA:</td><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';padding-left:5px">'.$this->metodoPago[2].'</td></tr></table></div><div style="font-weight:bold;font-size:11px;background:'.$this->color.';margin-top:10px">Observaciones</div><div style="font-size:10px;border:1px solid '.$this->color.';padding:5px"><div style="width:381px;padding-top:-2px">'.wordwrap($this->observaciones,112,"</div><div style=\"width:380px;margin-top:3px\">",true).'</div></div><div style="margin-top:10px"><table><tr><td style="font-weight:bold;font-size:11px;background:'.$this->color.';width:392px">Cadena original del complemento de certificacion del SAT</td><td style="font-weight:bold;font-size:11px;background:'.$this->color.';width:352px">Codigo original del complemento de certificacion del SAT</td></tr></table></div><div><table><tr><td style="font-size:8px;width:392px"><d iv><div>'.wordwrap($this->selloDigitales[0],58,"</div><div>",true).'</div></div><div style="font-weight:bold;font-size:11px;background:'.$this->color.';width:387px;margin-top:10px">Sello digital del emisor</div><div style="width:387px"><div>'.wordwrap($this->selloDigitales[1],58,"</div><div>",true).'</div></div><div style="font-weight:bold;font-size:11px;background:'.$this->color.';width:387px;margin-top:10px">Sello digital del SAT</div><div><div>'.wordwrap($this->selloDigitales[2],58,"</div><div>",true).'</div></div></td><td style="font-size:8px;width:352px;border-left:1px dashed '.$this->color.'"><img style="-webkit-user-select:none" src=""></td></tr></table></div><div style="font-size:8px;padding-left:100px"><table><tr><td>ESTE DOCUMENTO ES UNA REPRESENTACION IMPRESA DE UN CFDI</td><td style="padding-left:50px;border-collapse:collapse">Este Documento Fue Generado Utilizando Herramientas De Netwarmonitor</td></tr></table></div><div>Este Documento de creo con herramientas de Netwarmonitor</div> <div>'.'<div></div><div></div><div style="font-weight:bold;font-size:13px;background:'.$this->color.';padding-right:10px">T&eacute;rminos y condiciones</div><div style="font-size:11px;background:'.$this->color.';padding-right:10px; text-align:right;"></div><div><table style="border:1px '.$this->color.';"><tr><td style="font-size:10px; text-align: right; width:750px"></td></tr><tr></tr><tr></tr><tr><td style="font-size:10px; text-align: justify; width:750px">'.wordwrap('Pagar el 50% al hacer el pedido, y liquidar antes de enviar su mercanc&iacute;a o al recibirla por parte de la empresa. <br />Si requiere factura debe solicitarla al hacer el pedido con su vendedor. <br />Cambios o cancelaciones: Despu&eacute;s de haber realizado el pago se le har&aacute; un cargo de penalizaci&oacute;n seg&uacute;n sea el caso por gastos ocasionados por el mismo. <br />La entrega de la tina ser&aacute; de 2 a 8 d&iacute;as h&aacute;biles m&aacute;s lo que tarde el env&iacute;o a pie de casa. <br />Toda la mercanc&iacute;a viaja por cuenta y riesgo del comprador. <br />Garant&iacute;a de 3 a&ntilde;os en f&aacute;brica. <br /> <br />').'</td></tr></table></div><div></div><div></div><div></div>'.'</div></body></html>';
					break;
			}
