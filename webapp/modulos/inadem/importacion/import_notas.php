...
$data->read(dirname(__FILE__).'/notas_temp.xls');
...
for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) 
{
    ...
    $dato[2] = trim($data->sheets[0]["cells"][$i][2]); //tipo
    $dato[3] = trim($data->sheets[0]["cells"][$i][3]); //fecha
    $dato[4] = trim($data->sheets[0]["cells"][$i][4]); //rfc
    $dato[5] = trim($data->sheets[0]["cells"][$i][5]); //cliente
    $dato[6] = trim($data->sheets[0]["cells"][$i][6]); //uuid
    $dato[7] = trim($data->sheets[0]["cells"][$i][7]); //Folio Factura
    $dato[8] = trim($data->sheets[0]["cells"][$i][8]); //idVenta
    $dato[9] = trim($data->sheets[0]["cells"][$i][9]); //Total
    $dato[10] = trim($data->sheets[0]["cells"][$i][10]); //origen
    $dato[11] = trim($data->sheets[0]["cells"][$i][11]); //estatus
    $dato[12] = trim($data->sheets[0]["cells"][$i][12]); //aportacion inadem
    $dato[13] = trim($data->sheets[0]["cells"][$i][13]); //aportacion
    $dato[14] = trim($data->sheets[0]["cells"][$i][14]); //importe de nota
   /* $dato[15] = trim($data->sheets[0]["cells"][$i][15]); //Nombre Empresario
    ...
    if($dato[3] != '0' && $dato[4] != '0' && $dato[14] != '0' && $dato[13] != '0'){
        $r = $this->clienteModel->guardarLayNotas($dato);
    }
    else
    {
...
print_r($dato);
unlink(dirname(__FILE__).'/notas_temp.xls');
/*if(intval($sigue))
{
    //Validaciones
