$(document).ready(function() {
  ...
  $("#organismo").change(function(){
    obtenerInformacion($("#organismo").val(), 0, "");
  });

  $("#consultor").change(function(){
    obtenerInformacion($("#organismo").val(), $("#consultor").val(), "");
  });

  $("#folio").change(function(){
    obtenerInformacion($("#organismo").val(), $("#consultor").val(), $("#folio").val());
  });

  obtenerInformacion(0, 0, "");
});

function obtenerInformacion(organismo, consultor, folio){
  $.ajax({
    type: "POST",
      url: "../inovekia_dashboard/ajax.php?c=reporte&f=cursosFolio",
      dataType: "json",
      data: { id_organismo: organismo, id_consultor: consultor, folio: folio },
        ...
        if(respuesta.status !== undefined && respuesta.status == true){
          ...
          if(organismo == 0){
            ...
            if(consultor == 0){
              ...
              $("#folio").html(respuesta.f_folio);
            }
          }
          $(".agregado").remove();

          $.each(respuesta.folios, function(nombre, porcentajes){
