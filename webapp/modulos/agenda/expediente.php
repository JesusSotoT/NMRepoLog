		...
		buttons:{
			"Agregar": function(){
				...
				$.ajax({
					url:'form.php',
					type: 'POST',
					data: {funcion:'agregarevento',cliente:$("#cliente").val(),grupo:$("#grupo").val(),titulo:$("#titulo").val(),descripcion:$("#descripcion").val(),inicio:$("#inicio").val(),fin:$("#fin").val()},
					success: function(resp){ //$('#calendar').fullCalendar( 'refetchEvents' );
						if(resp==3){ alert("Estas tratando de ingresar una cita que se transpapela con otra , checa tu disponibilidad"); return false;}
					loadexpediente();
