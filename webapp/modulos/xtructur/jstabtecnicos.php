...
$(function() {
      ...
      jQuery("#jq_alta_categoria").jqGrid({
        ...
        datatype: "json",
        colNames:['Familia','Clave categoria','Categoria','Salario mensual','Dias trabajados por semana','Salario Diario'],
        colModel:[
      ...
      jQuery("#jq_alta_categoria").jqGrid('navGrid',"#jqp_alta_categoria",
        ...
        {beforeShowForm: function(form){ 
            ...
            $('#precio_venta').numeric(); 
               $('#sal_mensual').numeric(); 
               $("#dias").keyup(function(){
                  $('#sal_dia').val($('#sal_mensual').val()/($('#dias').val()*4));
                     var num = parseFloat($('#sal_dia').val());
              $('#sal_dia').val(num.toFixed(2));
            });
                     $("#sal_mensual").keyup(function(){
                  $('#sal_dia').val($('#sal_mensual').val()/($('#dias').val()*4));
                     var num = parseFloat($('#sal_dia').val());
              $('#sal_dia').val(num.toFixed(2));
            });
