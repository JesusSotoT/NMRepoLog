...
exit(); */
			...
			while($listaf=$buscaf->fetch_array(MYSQLI_ASSOC)){
				...
				<td align="center">
					<div onclick="comprobante('.$listaf['id'].');">
...
//								$upde=$conection->query("insert into mrp_stock (idProducto,cantidad,idAlmacen,idUnidad) values(".$producto.",".$cantidad.",".$almadestino.",".$unidad.");");
								$sql ='INSERT into movimientos_mercancia(idAlmacenOrigen,cantidadtotalOrigen,cantidadmovimiento,idUnidad,idProducto,idAlmacenDestino,cantidadtotalDestino,fechamovimiento,idEmpleado)';
								$sql.='values ("'.$value->almorigen.'","'.$value->cantalmorigen.'","'.$value->canti.'","'.$value->uni.'","'.$value->idProducto.'","'.$value->almdestino.'","'.$cantidadAlamcenDes.'","'.date('Y-m-d H:i:s').'","'.$_SESSION['accelog_idempleado'].'")';
