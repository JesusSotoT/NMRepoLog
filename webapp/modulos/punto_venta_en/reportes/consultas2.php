		...
		$elije=$_REQUEST['elije'];
			...
			$fecha=date('Y-m-d ' );
				...
				$fechabuscar=$nuevafecha;//7 dias
					...
					if($elije==1){ $linea=$_REQUEST['lifade']; 
 $consulta=$conection->query("select  v.estatus,v.idVenta,v.fecha,p.nombre,l.nombre filtro,s.nombre sucu,
vp.cantidad,vp.preciounitario,vp.subtotal,vp.impuestosproductoventa,vp.montodescuento,vp.descuento,vp.tipodescuento, vp.total
from mrp_linea l,mrp_producto p,venta_producto vp, venta v,mrp_sucursal s
where v.estatus=1 and l.idLin=".$linea." and p.idLinea=l.idLin and 
...
elseif($elije==2){ $familia= $_REQUEST['lifade'];
$consulta=$conection->query("select  v.estatus,v.idVenta,v.fecha,p.nombre,f.nombre filtro,s.nombre sucu,
vp.cantidad,vp.preciounitario,vp.subtotal,vp.impuestosproductoventa,vp.montodescuento,vp.descuento,vp.tipodescuento, vp.total
from mrp_linea l,mrp_producto p,venta_producto vp, venta v,mrp_familia f,mrp_sucursal s
where v.estatus=1 and l.idFam=f.idFam and f.idFam=".$familia." and p.idLinea=l.idLin and 
vp.idProducto=p.idProducto and vp.idVenta=v.idVenta and s.idSuc=v.idSucursal ".$sucursal." and
v.fecha BETWEEN '".$fechabuscar." 1:00:00' and '".$fecha." 23:00:00'"); 
}
elseif($elije==3){ $departamento= $_REQUEST['lifade'];
$consulta=$conection->query("select  v.estatus,v.idVenta,v.fecha,p.nombre,d.nombre filtro,s.nombre sucu,
vp.cantidad,vp.preciounitario,vp.subtotal,vp.impuestosproductoventa,vp.montodescuento,vp.descuento,vp.tipodescuento, vp.total
from mrp_linea l,mrp_producto p,venta_producto vp, venta v,mrp_familia f,mrp_departamento d,mrp_sucursal s
where  v.estatus=1 and l.idFam=f.idFam and f.idDep=d.idDep and d.idDep=".$departamento." and p.idLinea=l.idLin and 
vp.idProducto=p.idProducto and vp.idVenta=v.idVenta and s.idSuc=v.idSucursal ".$sucursal." and
v.fecha BETWEEN '".$fechabuscar." 1:00:00' and '".$fecha." 23:00:00'");
 }
...
vp.cantidad,vp.preciounitario,vp.subtotal,vp.impuestosproductoventa,vp.montodescuento,vp.descuento,vp.tipodescuento, vp.total
from mrp_linea l,mrp_producto p,venta_producto vp, venta v,mrp_sucursal s
where v.estatus=1 and l.idLin=".$linea." and p.idLinea=l.idLin and s.idSuc=v.idSucursal ".$sucursal." and 
vp.idProducto=p.idProducto and vp.idVenta=v.idVenta");  }
...
vp.idProducto=p.idProducto and vp.idVenta=v.idVenta and s.idSuc=v.idSucursal ".$sucursal);
	...
	if($elije==1 && $fechaelije!=4 && $fechaelije!=5 && $fechaelije!=3){ $linea=$_REQUEST['lifade']; 
 $consulta=$conection->query("select  v.estatus,v.idVenta,v.fecha,p.nombre,l.nombre filtro,s.nombre sucu,
vp.cantidad,vp.preciounitario,vp.subtotal,vp.impuestosproductoventa,vp.montodescuento,vp.descuento,vp.tipodescuento, vp.total
from mrp_linea l,mrp_producto p,venta_producto vp, venta v,mrp_sucursal s
where v.estatus=1 and l.idLin=".$linea." and p.idLinea=l.idLin and 
...
elseif($elije==2 && $fechaelije!=4 && $fechaelije!=5){ $familia= $_REQUEST['lifade'];
$consulta=$conection->query("select  v.estatus,v.idVenta,v.fecha,p.nombre,f.nombre filtro,s.nombre sucu,
vp.cantidad,vp.preciounitario,vp.subtotal,vp.impuestosproductoventa,vp.montodescuento,vp.descuento,vp.tipodescuento, vp.total
from mrp_linea l,mrp_producto p,venta_producto vp, venta v,mrp_familia f,mrp_sucursal s
where v.estatus=1 and l.idFam=f.idFam and f.idFam=".$familia." and p.idLinea=l.idLin and 
...
elseif($elije==3 && $fechaelije!=4 && $fechaelije!=5){ $departamento= $_REQUEST['lifade'];
$consulta=$conection->query("select  v.estatus,v.idVenta,v.fecha,p.nombre,d.nombre filtro,s.nombre sucu,
vp.cantidad,vp.preciounitario,vp.subtotal,vp.impuestosproductoventa,vp.montodescuento,vp.descuento,vp.tipodescuento, vp.total
from mrp_linea l,mrp_producto p,venta_producto vp, venta v,mrp_familia f,mrp_departamento d,mrp_sucursal s
where  v.estatus=1 and l.idFam=f.idFam and f.idDep=d.idDep and d.idDep=".$departamento." and p.idLinea=l.idLin and 
...
<tr style="background:#c4c4c4;color: #333" aling="center"><td></td><td></td><td></td><td></td>
	...
	<td height="30%" colspan="12" align="left" style="font-size:14px; text-align: right;	height: 34px; font-weight:bold"> 
	TOTAL DE VENTA $'.number_format($total, 2, '.', ',').'</td >
</tr>
...
';
	...
	echo '->
	<tr style="background:#c4c4c4;color: #333">
	<td colspan="12" align="center" style="font-size: 14px;font-weight:bold">No se encontraron ventas que coincidieran con su búsqueda.</td >
</tr>
	';
      }
		break;
...
elseif($elije==3){ $departamento= $_REQUEST['lifade'];
$consulta=$conection->query("select  v.estatus,v.idVenta,v.fecha,p.nombre,d.nombre filtro,s.nombre sucu,
vp.cantidad,vp.preciounitario,vp.subtotal,vp.impuestosproductoventa,vp.montodescuento,vp.montodescuento,vp.descuento,vp.tipodescuento, vp.total
from mrp_linea l,mrp_producto p,venta_producto vp, venta v,mrp_familia f,mrp_departamento d,mrp_sucursal s
where  v.estatus=1 and l.idFam=f.idFam and f.idDep=d.idDep and d.idDep=".$departamento." and p.idLinea=l.idLin and 
vp.idProducto=p.idProducto and vp.idVenta=v.idVenta and s.idSuc=v.idSucursal ".$sucursal." and
v.fecha BETWEEN '".$fechaini." 1:00:00' and '".$fechafin." 23:00:00'");
...
$proveedor=$_REQUEST['idprove'];
			...
			$fecha=date('Y-m-d ' );
				...
				$nuevafecha = date ( 'Y-m-d ' , $nuevafecha );
				$fechabuscar=$nuevafecha;//7 dias
				$consulta=$conection->query(" select v.estatus,v.idVenta,v.fecha,p.nombre,prove.razon_social,s.nombre sucu,
vp.cantidad,vp.preciounitario,vp.subtotal,vp.impuestosproductoventa,vp.montodescuento,vp.descuento,vp.tipodescuento, vp.total
from mrp_producto p,venta_producto vp, venta v,mrp_proveedor prove,mrp_sucursal s
where v.estatus=1 and p.idProveedor=prove.idPrv and prove.idPrv=".$proveedor." and
vp.idProducto=p.idProducto and vp.idVenta=v.idVenta and s.idSuc=v.idSucursal ".$sucursal." and
v.fecha BETWEEN '".$fechabuscar." 1:00:00' and '".$fecha." 23:00:00'"); 	
...
vp.cantidad,vp.preciounitario,vp.subtotal,vp.impuestosproductoventa,vp.montodescuento,vp.descuento,vp.tipodescuento, vp.total
from mrp_producto p,venta_producto vp, venta v,mrp_proveedor prove,mrp_sucursal s
where v.estatus=1 and p.idProveedor=prove.idPrv and prove.idPrv=".$proveedor." and s.idSuc=v.idSucursal ".$sucursal." and
vp.idProducto=p.idProducto and vp.idVenta=v.idVenta ");  }
...
where v.estatus=1 and vp.idVenta=v.idVenta and f.idFormapago=vp.idFormapago 
and vpro.idVenta=v.idVenta and f.idFormapago=".$forma." and s.idSuc=v.idSucursal ".$sucursal." and v.fecha 
BETWEEN '".$fechabuscar." 1:00:00' and '".$fecha." 23:00:00' GROUP  BY v.idVenta"); 	
...
where v.estatus=1 and vp.idVenta=v.idVenta and f.idFormapago=vp.idFormapago 
and vpro.idVenta=v.idVenta and f.idFormapago=".$forma." and s.idSuc=v.idSucursal ".$sucursal." and v.fecha 
BETWEEN '".$fechaini." 1:00:00' and '".$fechafin." 23:00:00' GROUP  BY v.idVenta");  }
	elseif($fechaelije==5){
...
from venta_pagos vp,venta v,forma_pago f,venta_producto vpro,mrp_sucursal s
where v.estatus=1 and vp.idVenta=v.idVenta and f.idFormapago=vp.idFormapago 
and s.idSuc=v.idSucursal ".$sucursal." and vpro.idVenta=v.idVenta and f.idFormapago=".$forma."  GROUP  BY v.idVenta ");  }
...
where v.estatus=1 and vp.idVenta=v.idVenta and f.idFormapago=vp.idFormapago 
and vpro.idVenta=v.idVenta and f.idFormapago=".$forma." and s.idSuc=v.idSucursal ".$sucursal." and v.fecha 
BETWEEN '".$fechabuscar." 1:00:00' and '".$fechabuscar." 23:00:00' GROUP  BY v.idVenta"); 	
		}
	///////////////////////////////////////////////////////////////////////
//  
// echo
...
echo '->
	<tr style="background:#c4c4c4;color: #333" >
	<td height="30%" colspan="6"  style="font-size:14px; text-align: center;	height: 34px; font-weight:bold"> 
	TOTAL DE FORMA DE PAGO $'.number_format($total, 2, '.', ',').'</td >
	</tr>
...
echo '
	...
	<td height="30%" colspan="6" align="center" style="font-size:14px; text-align: center;	height: 34px; font-weight:bold"> 
	TOTAL DE FORMA DE PAGO $'.number_format($total, 2, '.', ',').'</td >
	</tr>
';
