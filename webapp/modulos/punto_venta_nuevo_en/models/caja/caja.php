...
class cajaModel extends Connection {
    ...
    function usuarioSucursal($empleado){
        $selct = "SELECT idSuc FROM administracion_usuarios adm, accelog_usuarios acc WHERE acc.usuario = adm.nombreusuario and acc.idempleado = " . $empleado;
    ...
    function formasDePago(){
        ...
        $selct = "SELECT * from forma_pago where claveSat='01' or claveSat='02' or claveSat='04' or claveSat='07' or claveSat='28'";
        $res = $this->queryArray($selct);

        return $res['rows'];
    }

    function formasDePagoEn(){


        $selct = "SELECT * from forma_pago_en";
        $res = $this->queryArray($selct);
    ...
    foreach ($materilaes["rows"] as $key => $materialValue) {
        ...
        if ($_SESSION["simple"] == true && $result["rows"][0]["tipo_producto"] != 6) {
            ...
            if (((float) ($materialValue["cantidad"]*$cantidadInicial) > (float) (($resultqueryExistencia["rows"][0]["cantidad"] - $cantidad) - $resultqueryExistencia["rows"][0]["ocupados"])) || ((float) $resultqueryExistencia["rows"][0]["cantidad"] < (float) $cantidadInicial)) {
                ...
                if ($comanda == FALSE) {
                    throw new Exception("Insufficient amount of goods for this product", 1);
                }
            }
        }
...
function calculaImpuestosMysql($id,$cantidad,$impuestos,$elimina=0){
   ...
   // include "../../modulos/punto_venta_nuevo_en/conexionomar.php";
...
function calculaImpuestos_nuevaaa($idProducto,$comanda){
       ...
       foreach ($_SESSION['caja'] as $key => $value) {
           if($key!='cargos'){
                    ...
                    $queryImpuestos .= " where p.idProducto=" . $key . " and i.idSuc = " . $id_sucursal .  " and i.id=pi.idImpuesto ";
...
function calculaImpuestos($idProducto, $comanda, $descuento = 0.00, $precionuevo = 0.00) {
        ...
        //print_r($_SESSION["caja"]);
          ...
          $_SESSION["caja"]["cargos"]["impuestos"]['Tax'] = 0.0;
          $_SESSION["caja"]["cargos"]["impuestos"]['IEPS'] = 0.0;
          $_SESSION["caja"]["cargos"]["impuestos"]['test'] = 0.0;
          //echo 'sgundo='.$ieps;


          $selct = "SELECT idSuc FROM administracion_usuarios adm WHERE adm.idempleado = " . $_SESSION["accelog_idempleado"];

          $q_empleado_sucursal = $this->queryArray($selct);

          //$id_sucursal = $q_empleado_sucursal["rows"][0]["idSuc"];
          $id_sucursal = 100;
          $_SESSION["idsuc"]=$id_sucursal;

          if ($q_empleado_sucursal["total"] > 0) {
                $id_sucursal = $q_empleado_sucursal["rows"][0]["idSuc"];
                $_SESSION["idsuc"]=$id_sucursal;
          } else {
                throw new Exception("No existe una sucursal.", 1);
          }
          foreach ($_SESSION["caja"] as $key => $value) {
            if ($key != 'cargos') {
                if ($key == $idProducto) {
            //        echo 'terccer='.$faieps;
                    ...
                    $queryImpuestos .= " from impuesto i, mrp_producto p ";
                    $queryImpuestos .= " left join producto_impuesto pi on p.idProducto=pi.idProducto ";
                    $queryImpuestos .= " where p.idProducto=" . $idProducto . " and i.idSuc = " . $id_sucursal .  " and i.id=pi.idImpuesto ";
                    ...
                    foreach ($resultImpuestos["rows"] as $key => $valueImpuestos) {
                     ...
                     /*   if(ieps=0)
                        ...
                        if($valueImpuestos["retenido"]!=1){
                            ...
                            $_SESSION["caja"]["cargos"]["impuestos"][$valueImpuestos["nombre"]] = $producto_impuesto;
                        }
    ...
    function eliminaProducto($idProducto) {
       ...
       // $this->eliminaProductoMysql($idProducto);
        ...
        $_SESSION["caja"]["cargos"]["impuestos"]["Tax"] = 0;
...
function getTimeZone($idEmpleado) {
    $selct = "SELECT tz.tz_description FROM administracion_usuarios adm, mrp_sucursal suc, mrp_timezones tz WHERE adm.idempleado = " . $ide . " AND adm.idSuc = suc.idSuc AND suc.tm_timezone = tz.id";
    $res = $this->queryArray($selct);

    return $res['tz.tz_description'];
...
function guardarVenta($cliente, $idFact, $documento, $suspendida, $propina, $comentario) {
    ...
    try {
        ...
        //$tz = getBranchTimeZone($_SESSION["accelog_idempleado"]);
        ...
        foreach (@$_SESSION['caja'] as $key => $producto) {
            if ($key != 'cargos') {
                ...
                //Busca Total Correcto de Impuestos EGA
                $simpuestosdetalle=$simpuestosdetalle+$impuestos;
...
//echo   $selectProduccion;
            ...
            //Actualizacion de impuestos a partir del detalle EGA
            $sqlActimventa="";
            $sqlActimventa="update venta set montoimpuestos=$simpuestosdetalle where idventa=$idVenta";
            $resultsqlActimventa = $this->queryTrans($sqlActimventa);
...
/*-------------------------------------------------------*/
    ...
    $hasFood = $resFood['rows'][0]['food'];
        ...
        $queryFormaPago = " SELECT name nombre,referencia,claveSat from venta_pagos vp inner join forma_pago fp on vp.idFormapago = fp.idFormapago where vp.idVenta=" . $idVenta;
       ...
       // $parametros['DatosCFD']['Total'] = $parametros['DatosCFD']['Total'] - 0.01;
        ...
        foreach ($_SESSION['caja'] as $key => $producto) {
            if ($key != 'cargos') {
                ...
                $queryImpuestos .= " from impuesto i, mrp_producto p ";
                $queryImpuestos .= " left join producto_impuesto pi on p.idProducto=pi.idProducto ";
                $queryImpuestos .= " where p.idProducto=" . $producto->id . " and i.idSuc = " . $id_sucursal .  " and i.id=pi.idImpuesto ";
                $queryImpuestos .= " Order by pi.idImpuesto DESC ";
                //echo $queryImpuestos;
                $resultImpuestos = $this->queryArray($queryImpuestos);
...
//        unset($_SESSION['caja']);
        ...
        if($consumo == 1 || $consumo=='1'){
           ...
           // $subTotalCon = $parametros['DatosCFD']['Total'] - $ivaCon;
            ...
            $nn2["Tax"]["16.00"]["Valor"] = $elIva;
            //echo 'sub'.$parametros['DatosCFD']['Subtotal'];
        }
        /* CORREO RECEPTOR
        ============================================================== */
        if ($nn2 == '') {
            $nn2["Tax"]["0.0"]["Valor"] = 0.00;
        }
        $nn = $nn2;
        $azurian['nn']['nn'] = $nn;
        ...
        if ($haytras == 1) {
            ...
            $traslads.='0.00';
            $trasladsimp = '0.00';
            $iva.="<cfdi:Traslados><cfdi:Traslado impuesto='Tax' tasa='0.00' importe='0.00' /></cfdi:Traslados>";
        }
        if ($hayret == 1) {
        ...
        /*  foreach ($impuestosDatos as $key => $value) {
          ...
          if($value['TipoImpuesto']=='Tax' || $value['TipoImpuesto']=='Tax' || $value['TipoImpuesto']=='Tax'){
          $iva="<cfdi:Traslados><cfdi:Traslado impuesto='Tax' tasa='16' importe='";
         ...
         */
          ...
          $azurian['Impuestos']['Tax'] = $traslads . '|' . number_format($trasladsimp, 2, '.', '');
  ...
  function facturarRecibo($idFact, $idVenta, $bloqueo, $mensaje) {
    ...
    $queryFormaPago = " select name nombre,referencia from venta_pagos vp inner join forma_pago fp on vp.idFormapago = fp.idFormapago where vp.idVenta=" . $idVenta;
...
function envioFactura($uid, $Email, $azurian, $doc) {
    ...
    $obj->agregarMetodo($formapago, '', $azurian['Basicos']['total']);
