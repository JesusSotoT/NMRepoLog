...
$ticket_sucursal=datossucursal($_SESSION["accelog_idempleado"]);
...
<div id="receipt_wrapper">
	...
	<div id="receipt_header" style="text-align:center;">
		...
		<div id="company_address"><?php echo utf8_decode($ticket_sucursal->address);?></div>
	...
	<div id="sale_receipt">Sales ticket</div>
	<div id="sale_time"><!--Fecha y hora--><?php echo formato_fecha($venta->fecha);?></div>
</div>

<div id="receipt_general_info" style="text-align:center;">
	<div id="sucursal">Branch:<?php echo $_SESSION["sucursalNombre"]; ?></div>
	<div id="customer">Customer:<?php echo $venta->cliente; ?></div>
	<div id="sale_id">Id Sale:<?php  echo $venta->folio; ?></div>
	<div id="employee">Employee:<?php  echo $venta->empleado; ?></div>
...
<table id="receipt_items" border='0'>
	...
	<?php $total=0; while($producto=mysql_fetch_object($productos)){
		...
		$qi=mysql_query("select pi.idimpuesto,i.nombre as impuesto, pi.valor from producto_impuesto pi
											left join impuesto i on i.id=pi.idImpuesto where i.idSuc=$idsuc and pi.idProducto=".$producto->idProducto." order by pi.idimpuesto DESC");
		...
		if($producto->montodescuento>0){
			?>
			<tr>
				<td style='text-align:center;'>Disc:</td><td style='text-align:center;'>$<?php echo number_format( $producto->montodescuento,2,".",","); ?></td>
			</tr>
			<?php
		}
	}
...
<!--
	...
	<tr>
		<!--<td colspan="1" style="text-align:right;">Payment method:</td>-->
	...
	<tr>
		<td colspan="2" style='text-align:right;'><b>Change</b></td>
