<?php
	...
	class Seguridad extends Connectionapi {
		...
		function generaToken($dispositivo, $usuario){
			...
			$consulta = " SELECT id FROM api_token WHERE token = :token;";
			$resultados = $this->queryArray($consulta, array("token" => $token));
			if($resultados["total"] > 0){
				return $this->generaToken($dispositivo, $usuario, $inicio, $fin);
			}else{
				$consulta = " UPDATE api_token SET activo = 0 WHERE dispositivo = :dispositivo AND activo = 1;";
				...
				$consulta = " INSERT INTO api_token (id, id_empleado, dispositivo, token, inicio, fin) VALUES ";
		...
		function validaToken($dispositivo, $token){
			$consulta = " SELECT id, id_empleado AS empleado, dispositivo, inicio, fin FROM api_token WHERE token = :token AND activo = 1;";
			$resultados = $this->queryArray($consulta, array("token" => $token));
			if($resultados["total"] == 1){
				...
				if(crypt($texto, $token) == $token){
					...
					if(strtotime($registro["fin"]) < strtotime(date("Y-m-d H:i:s"))){
						$consulta = " UPDATE api_token SET activo = 0 WHERE id = :id;";
		...
		function validaUsuario($usuario, $contrasena, $id = null){
			...
			$query = '  SELECT e.idempleado, u.clave, au.idSuc, up.idperfil, au.correoelectronico FROM empleados AS e 
                            ...
                            if($id == null){
                            	$query .= 'WHERE u.usuario = :usuario AND e.visible = -1 AND (up.idperfil = 2 OR up.idperfil = 3) ';
            ...
            if($result["total"] == 1){
            	if($result["rows"][0]["clave"] != $contrasena){
		            ...
		            $result["rows"][0]["facturacion"] = $tipo_facturacion;
            	}
		...
		function terminaUsuario($token){
			$consulta = " UPDATE api_token SET activo = 0 WHERE id = ". $this->Sesion .";";
