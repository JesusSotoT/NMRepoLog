<?php
	...
	class Seguridad extends aConnection {
		...
		function validaParametros($parametros, $validaciones){
			foreach ($validaciones as $index => $validacion) {
				if((!isset($parametros[$index]) && !$validacion["nulo"]) || (isset($parametros[$index]) && $parametros[$index] == "" && !$validacion["vacio"])){
					return array("status" => false, "mensaje" => "El parametro ::". $index .":: no puede ser vacio o nulo");
				}
				else if(isset($parametros[$index])){
		...
		function generaToken($dispositivo, $usuario){
			...
			$consulta = " SELECT id FROM api_token_hibrido WHERE token = :token;";
			$resultados = $this->queryArray($consulta, array("token" => $token));
			if($resultados["total"] > 0){
				return $this->generaToken($dispositivo, $usuario, $inicio, $fin);
			}else{
				$consulta = " UPDATE api_token_hibrido SET activo = 0 WHERE dispositivo = :dispositivo AND activo = 1;";
				...
				$consulta = " INSERT INTO api_token_hibrido (id, id_empleado, dispositivo, token, inicio, fin) VALUES ";
		...
		function validaToken($dispositivo, $token){
			$consulta = " SELECT id, id_empleado AS empleado, dispositivo, inicio, fin FROM api_token_hibrido WHERE token = :token AND activo = 1;";
			$resultados = $this->queryArray($consulta, array("token" => $token));
			if($resultados["total"] == 1){
				...
				if(crypt($texto, $token) == $token){
					...
					if(strtotime($registro["fin"]) < strtotime(date("Y-m-d H:i:s"))){
						$consulta = " UPDATE api_token_hibrido SET activo = 0 WHERE id = :id;";
		...
		function validaUsuario($usuario, $contrasena, $id = null){
			require('libraries/encriptacion.php');
			require('webconfig.php');
			$encriptador = new Encriptacion();
			$contrasena = $this->fencripta($contrasena, $accelog_salt);
			$usuario = str_replace("'", "", $usuario);
			$usuario = str_replace("=", "", $usuario);
			$usuario = str_replace("\\", "", $usuario);

			$query = '  SELECT e.idempleado, u.clave FROM empleados AS e 
                            ...
                            if($id == null){
                            	...
                            	$query .= 'WHERE e.idempleado = :id AND e.visible = -1 ';
            ...
            if($result["total"] == 1){
            	if($result["rows"][0]["clave"] != $contrasena){
	            	...
	            	$result["rows"][0]["ib_dn"] = $encriptador->encriptar($bd);
	            	$result["rows"][0]["ib_du"] = $encriptador->encriptar($usuariobd);
	            	$result["rows"][0]["ib_dc"] = $encriptador->encriptar($clavebd);
            	}
            }else{
            	$result = $incorrecto;
            }
		...
		function terminaUsuario($token){
			$consulta = " UPDATE api_token_hibrido SET activo = 0 WHERE id = ". $this->Sesion .";";
